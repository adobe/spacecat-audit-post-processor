...
export function buildExperimentationSlackMessage(url, auditResult) {
  ...
  Object.entries(groupedData).forEach(([key, value]) => {
    ...
    for (let i = 0; i < Math.min(3, value.length); i += 1) {
      variantstats.push(markdown(`*Variant:* ${value[i].variant} | *Period:* ${value[i].time5} - ${value[i].time95} | *Events:* ${value[i].variant_experimentations} | *Conversions:* ${value[i].variant_conversions} | *Confidence:* ${value[i].p_value} | *Conversion Rate:* ${value[i].variant_conversion_rate}`));
    }
    const stats = section({
      fields: variantstats,
    });
