...
describe('GithubClient', () => {
  ...
  describe('fetchGithubDiff', () => {
    ...
    it('skips github diff calculation if no github repo was defined for the provided site', async () => {
      const baseUrl = 'https://example.com';
      const client = GithubClient({ baseUrl: 'https://api.github.com', gitHubId: 'id', gitHubSecret: 'secret' });
      const logStub = sandbox.stub(console, 'info');

      await client.fetchGithubDiff(baseUrl, '', '', '');
      expect(logStub.calledWithMatch(`No github repo defined for site ${baseUrl}. Skipping github diff calculation`)).to.be.true;
    });
  });
});
